...
// 
bool twaiSendUint16(uint32_t id, uint16_t value, int max_retries = 3) {
  uint8_t b0 = (uint8_t)((value >> 8) & 0xFF); // MSB
  uint8_t b1 = (uint8_t)(value & 0xFF);        // LSB

  Serial.printf("TX attempt: ID=0x%03X value=%u -> bytes: %u(0x%02X) %u(0x%02X)\n",
                id, (unsigned int)value, b0, b0, b1, b1);
...
void loop() {
  ...
  if ((now - lastSendMs) >= SEND_INTERVAL_MS) {
    ...
    bool ok = twaiSendUint16(ID_TX, dist_mm);
